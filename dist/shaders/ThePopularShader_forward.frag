...
vec3 squaresColours(vec2 p)
{
	...
	// Assign different frequencies to different squares based on position
	// Use grid position to select frequency band: bass, speech, presence, brilliance
	int freqBand = int(mod(p.x * 8.0, 4.0));
	float freqAmp = 0.0;
	vec3 freqColor = vec3(1.0);
	
	if (freqBand == 0) {
		// Bass frequencies - red tint
		freqAmp = fft.x;
		freqColor = vec3(1.0, 0.3, 0.2);
	} else if (freqBand == 1) {
		// Speech frequencies - green tint  
		freqAmp = fft.y;
		freqColor = vec3(0.2, 1.0, 0.3);
	} else if (freqBand == 2) {
		// Presence frequencies - blue tint
		freqAmp = fft.z;
		freqColor = vec3(0.3, 0.5, 1.0);
	} else {
		// Brilliance frequencies - yellow tint
		freqAmp = fft.w;
		freqColor = vec3(1.0, 0.8, 0.2);
	}
	
	c += freqAmp * freqColor * 0.5;
...
vec3 scene(vec2 p)
{
	mat3 cam = rotateXMat(cos(time * 0.2) * 0.1) * rotateYMat(0.0);

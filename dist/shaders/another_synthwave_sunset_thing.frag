...
float hash(vec2 uv){
    ...
    return (a>0.?
        ...
        :0.)-(textureMirror(iAudioData,vec2((uv.x*29.+uv.y)*.03125,1.)).x)*audio_vibration_amplitude;
}

float edgeMin(float dx,vec2 da, vec2 db,vec2 uv){
    uv.x+=5.;
    vec3 c = fract((round(vec3(uv,uv.x+uv.y)))*(vec3(0,1,2)+0.61803398875));
    float a1 = textureMirror(iAudioData,vec2(c.y,0.)).x>.6?.15:1.;
    float a2 = textureMirror(iAudioData,vec2(c.x,0.)).x>.6?.15:1.;
    float a3 = textureMirror(iAudioData,vec2(c.z,0.)).x>.6?.15:1.;
...
vec3 gsky(vec3 rd,vec3 ld,bool mask){
    ...
    //float st = mask?pow512(texture(iAudioData,(rd.xy+vec2(300.1,100)*rd.z)*10.).r)*(1.-min(haze,1.)):0.;
    //float st = mask?pow512(hash21((rd.xy+vec2(300.1,100)*rd.z)*10.))*(1.-min(haze,1.)):0.;
    float st = mask?(starnoise(rd))*(1.-min(haze,1.)):0.;
    vec3 back = vec3(.4,.1,.7)*(1.-2.5*textureMirror(iAudioData,vec2(.5+.05*rd.x/rd.y,0.)).x
...
{
	float iTimeDelta =.01;//*(FFT(1)+FFT(25)+FFT(50)+FFT(75));
    fragColor=vec4(0);
    #ifdef AA
	...
	//float dt = fract(texture(iAudioData,float(AA)*(fragCoord+vec2(x,y))/iChannelResolution[0].xy).r+iTime)*shutter_speed;
    float dt = fract(hash21(float(AA)*(fragCoord+vec2(x,y)))+iAmplifiedTime)*shutter_speed;
    //jTime = mod(iTime-dt*iTimeDelta,4000.);
    jTime = mod(iAmplifiedTime-dt*iTimeDelta,4000.);

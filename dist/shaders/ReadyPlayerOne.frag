...
void main()
{
    vec4 p = vec4( 10., -.9 + sin(iAmplifiedTime), -iAmplifiedTime, 0 );
    ...
    for( int i = 0; i++ < 99; t=texture( iChannel1, p.xz * 0.0525 ) )
        ...
        t.b > 1.525 - texture(iAudioData, vec2( .04 * t.r * t.r * 30., 2.25*float(i) )).r  ? gl_FragColor += t*.015 : gl_FragColor;//+=sin(iAmplifiedTime)/10.;
    
    gl_FragColor *= gl_FragColor * vec4( .9, .8, 1.5*sin(iAmplifiedTime), .5 );

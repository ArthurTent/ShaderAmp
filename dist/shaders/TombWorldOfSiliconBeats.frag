...
float beat() {
  ...
  for(float x=.025;x<.2;x+=.025) {
    f=textureLod(iAudioData,vec2(x,.25),0.).x*(1.+.5*x);
...
float freq(float x) {
  x=fract(x)*.5+0.025;
  return textureLod(iAudioData,vec2(x,.25),0.).x*(1.+.5*x);
}
...
vec3 path(float z) {
 if(smoothstep(0.0, FFT(1), .63)==1.){
 	//camera_pos.x+=1.5;
	//camera_direction.x+=1.5;
 }
 //camera_pos.x+=smoothstep(0.0, FFT(1), .63);
 //camera_pos.x+=FFT(1)*2.;
...
vec4 render(vec2 p2, vec2 q2) {
  ...
  O=tanh_approx(O);
  O=max(O,0.);
  O*=smoothstep(0.,6., iAmplifiedTime-p2.y*p2.y);
  O=sqrt(O);
...
if(iTime<21.) {
	...
	if(snd>.53){ C*=1.5*rotM(sign(snd*5.)*aTime/17.);}
	else  if ( fTime<.32) C *= 1.5*rotM(sign(fract(aTime/64.)-1.75)*aTime/4.);
	else if (fract(aTime/32.)<.62) C=C; //NoOp
   	else if(snd>.4){C*=(1.0+snd*.25);}

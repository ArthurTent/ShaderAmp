...
void main(){
    ...
    vec4 bang = vec4(0);
    for (int n=0;n<4;n++)
        bang[n] = smoothstep( .91, 1., texelFetch( iAudioData, ivec2(n,0), 0 ).x);
    ...
    float bBarAmp = texelFetch( iAudioData, ivec2(1024.*abs(tmp.x/(2.*tmp.z)),0),0).x; // sound texture, full
    tmp = vec4( uv*winDistort+vec2(1.05,.0), vec2(.2,.4));
    float lBar = sdBox( tmp.xy, tmp.zw); //left "screen"
    float lBarAmp = texelFetch( iAudioData, ivec2(24.+71.*(tmp.y+tmp.w)/(2.*tmp.w),0),0).x; // sound texture, presence
    tmp.x = uv.x*winDistort-1.05;
    float rBar = sdBox( tmp.xy, tmp.zw); //right "screen"
    float rBarAmp = texelFetch( iAudioData, ivec2(95.+417.*(tmp.y+tmp.w)/(2.*tmp.w),0),0).x; // sound texture, brilliance
    ...
    float reticleAmp = (length(uv)<.1/fft.y+.1)? //inner/outer
        texelFetch(iAudioData,ivec2(6.+17.*rho,0),0).x : //sound texture, speech
        (tmp.x>.0)? //left/right
            texelFetch(iAudioData,ivec2(94.-71.*(tmp.x-.1-tmp.z)/.3,0),0).x : //sound texture, -presence
            texelFetch(iAudioData,ivec2(5.-5.*(-tmp.x-.1-tmp.z)/.3,0),0).x; //sound texture, -bass
    ...
    vec2 fuckCoord = vUv;
    vec2 fragCoord = fuckCoord;
    col -= clamp(-1.,.0,texelFetch( iAudioData, ivec2(512.*2.*abs(fragCoord.x/iResolution.x-.5)*winDistort,0),0).x

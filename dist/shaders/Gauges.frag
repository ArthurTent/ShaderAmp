...
void main(){
    ...
    for (float n=0.;n<4.;n++){ //lower left set of integrated four
        ...
        col += .002/max(.002,tmp.x) * getCol(colId+n) * (.2+.8*step( .91, texelFetch( iAudioData, ivec2(n,0), 0 ).x)); //light, bass channels
    }
    ...
    float wave = texelFetch( iAudioData, ivec2((tmp.x+tmp.z)*512./(2.*tmp.z)*.98+1.,1), 0).x; //get waveform from texture, trim edges
    if (wave == 0.){ //catch if no waveform found
        for (int n=0;n<9;n++) //synthesize crude approximation
            wave += texelFetch( iAudioData, ivec2(n,0), 0 ).x * sin((tmp.x+tmp.z)*PI/tmp.z * float(n*n*n) + aTime*2.*PI);
    ...
    for (int n=0;n<int(6.*fract(aTime));n++) //synthesize crude approximation
        wave += texelFetch( iAudioData, ivec2(n,0), 0 ).x * sin((tmp.x+tmp.z)*PI/tmp.z * float(n));
    ...
    col += tmp.y * texelFetch( iAudioData, ivec2(1024.*abs(tmp.x/(2.*tmp.z)),0),0).x  * getCol(colId+2.); //sound texture, full, mirrored

    tmp = vec4( uv+vec2(-.75,.18), vec2(.22,.13)); //spectrum bar top right
    tmp.y = aaStep(sdBox( tmp.xy, tmp.zw));
    col *= 1.-tmp.y; //delete col
    tmp.x = texelFetch( iAudioData, ivec2(6.+18.*(tmp.x+tmp.z)/(2.*tmp.z),0),0).x; //sound texture, speech
    col += tmp.x*tmp.x*tmp.x*tmp.x * tmp.y * getCol(colId+0.);

    tmp = vec4( uv+vec2(-.18,.1), vec2(.3,.05)); //spectrum bar top left
    tmp.y = aaStep(sdBox( tmp.xy, tmp.zw));
    col *= 1.-tmp.y; //delete col
    tmp.x = texelFetch( iAudioData, ivec2(24.+71.*(tmp.x+tmp.z)/(2.*tmp.z),0),0).x; //sound texture, presence
    col += tmp.x*tmp.x*tmp.x * tmp.y * getCol(colId+1.);

    tmp = vec4( uv+vec2(-.52,.5), vec2(.45,.02)); //spectrum bar bottom
    tmp.y = aaStep(sdBox( tmp.xy, tmp.zw));
    col *= 1.-tmp.y; //delete col
    col += texelFetch( iAudioData, ivec2(95.+417.*(tmp.x+tmp.z)/(2.*tmp.z),0),0).x * tmp.y * getCol(colId+2.); //sound texture, brilliance
    ...
    col *= 1.-tmp.z; //delete col
    tmp.xy /= 2.*tmp.w;
    col += (texelFetch( iAudioData, ivec2(int(512.*2.*abs(tmp.x)),0),0).x - 2.*abs(tmp.y)) * tmp.z * getCol(colId+1.); //mirrored sound texture spectrum

...
void main() {
   ...
   // uv = uv*curvature + uv*.935;
    ...
    scol[5]=texture(iAudioData, vec2(.17*5., 0.25) ).x; scol[5]=f1(clamp(1.*scol[5]*scol[5], 0., 1.)); scol[5]*=MM*scol[5]*scol[5];
...
#		define J .003
        ...
        vec3 reflectioncolor = vec3(texture(iAudioData, vec2(0., 0.25) ).x, texture(iAudioData, vec2(.17*4., 0.25) ).x, texture(iAudioData, vec2(.99, 0.25) ).x);
    ...
    gl_FragColor = vec4(pow(final_color, vec3(.85)), 1.);

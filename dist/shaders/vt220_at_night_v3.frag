...
float getShape(vec2 p, float r){ //shape combination scaling with r about 0,0
	...
	//float finalLine = Dancer(fract(p), newSeed-0.41);
	//finalLine *= mod(newSeed.y, 2.0);  // the mod kills every other line.
	//float lseed = length(newSeed);
	//return Dancer(fract(p), newSeed-0.41)*mod(newSeed.y, 2.);
	...
	/*
	return (fract(iAmplifiedTime/16.)<.5)? 
		//getRing(p,r)+.2*(1.-fft.z)*getTria(p/3.,r) : getTria(p,r)+.2*(1.-fft.z)*getRing(p/3.,r);
		//(1.-fft.z)*renderTeianeural(p) : 
		(1.-fft.z)*Dancer(fract(p), newSeed-0.41)*mod(newSeed.y, 2.) : 
			sin(iAmplifiedTime*.1)>.5? (1.-fft.z)*heart(p):
			(1.-fft.z)*funPattern(p.x,p.y, iAmplifiedTime, snd)*Dancer(fract(p), newSeed-0.41)*mod(newSeed.y, 2.);
		//(1.-fft.z)*heart(p);
	*/
}
...
void mainImage(out vec4 fragColor, in vec2 fragCoord )
{
	...
	//bgColor+=vec4(bg, 1.);
    //c = mix(vec4(bg, 1.), c, smoothstep(0.1, 0.2,c ));
    ...
    */
    // Define the heart shape using renderHeart
    //float heart = renderHeart(uv);

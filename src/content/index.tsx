import React, {useEffect, useState, useRef, MutableRefObject, Suspense} from 'react';
...
import { SETTINGS_SPEEDDIVIDER, SETTINGS_VOLUME_AMPLIFIER, SETTINGS_WEBCAM, SETTINGS_WEBCAM_AUDIO, STATE_CURRENT_SHADER, STATE_SHADERINDEX, STATE_SHADERNAME, STATE_SHOWSHADERCREDITS } from '@src/storage/storageConstants';
import "../css/app.css";
import css from "./styles.module.css";
import { defaultShader } from '@src/helpers/constants';
import ShaderPreloader, { useShaders } from './Components/ShaderPreloader';
...
const App: React.FC = () => {
    ...
    const [currentShaderIndex] = useChromeStorageLocal<number>(STATE_SHADERINDEX, 0);
            ...
            <Suspense fallback={<Loading/>}>
                <Canvas
                    ...
                    <OrthographicCamera makeDefault zoom={1}
                        ...
                        position={[0, 0, 1]}
                    />
                    <AnalyzerMesh analyser={analyser} canvas={renderCanvasRef.current} videoElement={videoRef.current}
                        shaderObject={currentShader} speedDivider={speedDivider} loadedMaterials={shaders} 
                        shaderIndex={currentShaderIndex}/>
                </Canvas>
                <video ref={videoRef} id={css.bgVideo} controls={false} muted
                    loop style={{visibility: 'hidden'}}></video>
